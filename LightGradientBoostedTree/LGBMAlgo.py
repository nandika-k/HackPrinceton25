...
def extract_acc(accel_xyz, fs):
    ...
    """
    return extract_acc(accel_xyz, fs)
...
def simulate_window(window_sec=20, fs_ecg=5, fs_accel=50, rng=None):
    ...
    if scen == "normal":
        acc += 0.2 * np.sin(2 * np.pi * 1.0 * t_accel)[:, None]
    elif scen == "run":
        acc += 0.7 * np.sign(np.sin(2 * np.pi * 2.0 * t_accel))[:, None]
    elif scen == "trip":
        impact_idx = int(0.5 * n_accel)
        acc[impact_idx:impact_idx+5] += 4.0
        acc += 0.5 * np.sin(2 * np.pi * 1.5 * t_accel)[:, None]
        ...
        cut = int(0.3 * n_accel)
        acc[:cut] += 0.3 * np.sin(2 * np.pi * 1.0 * t_accel[:cut])[:, None]
        acc[cut:] *= 0.05
    elif scen == "panic":
